...
import { ModeloPricingPage } from "./ui/pages/modelo-pricing/ModeloPricingPage";

function RootRedirect(){const {currentUser}=useAuth();return <Navigate to={currentUser?"/dashboard":"/login"} replace/>;}
function AppRoutes(){return <Routes>
  ...
  <Route element={<RequireAuth><AppShell/></RequireAuth>}>
    ...
    <Route path="/modelo-pricing" element={<RequireSection section="modeloPricing"><ModeloPricingPage/></RequireSection>}/>
    ...
    <Route path="/calculadora" element={<Navigate to="/nueva-cotizacion" replace/>}/>
  ...
  <Route path="/" element={<RootRedirect/>}/><Route path="*" element={<NotFound/>}/>
</Routes>;}
function NotFound(){return <div className="flex min-h-screen flex-col items-center justify-center gap-2 bg-ink-50 px-4 text-center"><p className="text-4xl">🧭</p><h1 className="text-lg font-semibold text-ink-900">Página no encontrada</h1><p className="text-sm text-ink-500">La ruta que buscas no existe en Price Model 365.</p><a href="/Pricing/#/" className="mt-2 text-sm font-medium text-brand-600 hover:underline">Volver al inicio</a></div>;}
function App(){const [ready,setReady]=useState(false);const [bootError,setBootError]=useState<string|null>(null);useEffect(()=>{try{ensureSeeded();}catch(error){console.error("[Price Model 365] Error inicializando datos demo",error);setBootError(error instanceof Error?error.message:"No fue posible inicializar los datos demo.");}finally{setReady(true);}},[]);if(!ready)return <div className="flex min-h-screen items-center justify-center bg-white text-sm text-slate-500">Cargando Price Model 365…</div>;if(bootError)return <div className="flex min-h-screen items-center justify-center bg-white p-6"><div className="max-w-lg rounded-xl border border-red-200 bg-red-50 p-5 text-red-900"><h1 className="font-semibold">No se pudo iniciar Price Model 365</h1><p className="mt-2 text-sm">{bootError}</p><p className="mt-3 text-xs">Recarga la página. Si el problema continúa, borra los datos del sitio para ic-academy.github.io y vuelve a intentar.</p></div></div>;return <HashRouter><ToastProvider><AuthProvider><AppRoutes/></AuthProvider></ToastProvider></HashRouter>;}
export default App;
