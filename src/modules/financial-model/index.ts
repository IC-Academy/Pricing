...
export const DEFAULT_FINANCIAL_MODEL_PARAMS: FinancialModelParams = {
  ...
  fxUsdMxn: 18,
  // Sin valores inventados: Pricing debe cargar estos costos desde su modelo real.
...
export function calculateFinancialScenario(input: FinancialScenarioInput, params: FinancialModelParams): FinancialScenarioResult {
  ...
  const totalServicio = r2(totalUnitario * posiciones);

  const est = ESTRUCTURA_CIUDAD_DEMO.find((e)=>e.ciudad===input.ciudad);
  const hcActual = est?.hc ?? 0;
  const hcProyectado = hcActual + posiciones;
  ...
  // Para reclutamiento usamos la referencia disponible en la tabla como capacidad observada.
  // Si el ratio no existe, no inferimos contratación automática.
  ...
  return {
    laboral:{ sueldo:salario, aguinaldo, vacaciones, primaVacacional, isn, riesgoTrabajo, otrasCargas, totalUnitario, totalServicio },
