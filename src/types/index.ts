...
export interface User { id:string; fullName:string; cargo?:string; email:string; role:RoleId; active:boolean; permissions?:PricingPermissions; createdAt:string; }
...
export interface CatalogHistoryEntry { id:string; catalogItemId:string; fecha:string; usuario:string; campoAnterior:Record<string,unknown>; campoNuevo:Record<string,unknown>; comentario?:string; }

export type NotificationSeverity = "info" | "warning" | "critical";
export interface NotificationItem { id:string; title:string; message:string; severity:NotificationSeverity; createdAt:string; read:boolean; relatedCatalogItemId?:string; channel:"in_app"|"outlook"|"teams"; }

export const CIUDADES_DEMO = ["CDMX", "Querétaro", "Guadalajara"] as const;
export type CiudadDemo = (typeof CIUDADES_DEMO)[number];
export const ESTADO_POR_CIUDAD: Record<CiudadDemo,string> = { CDMX:"Ciudad de México", "Querétaro":"Querétaro", Guadalajara:"Jalisco" };
...
export interface ParametrosComerciales {
  grossMarginObjetivo:number; vigenciaPropuestaDias:number; observaciones?:string; opcionales?:string;
  moneda?:MonedaCotizacion; tipoCambioUsdMxn?:number;
}

export interface DatosGenerales { cliente:string; nombreOportunidad:string; ciudad:CiudadDemo; estado:string; fecha:string; vendedorId:string; vendedorNombre:string; }
...
export interface PuestoCalculado extends PuestoCotizado {
  ...
  precioRecomendadoUnitario:number; precioTotalPuesto:number; precioAnualPuesto?:number;
}
...
export interface AuditLogEntry { id:string; entidad:AuditEntity; entidadId:string; descripcion:string; valorAnterior?:string; valorNuevo?:string; usuario:string; fecha:string; comentario?:string; }
export interface BenchmarkEntry { id:string; puesto:PerfilPuesto; ciudad:CiudadDemo; tarifaMercadoMensual:number; fuente:string; fechaActualizacion:string; fechaVencimiento:string; diasAnticipacionAlerta:number; responsable:string; activo:boolean; comentarios?:string; }
