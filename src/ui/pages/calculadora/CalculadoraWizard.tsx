...
import { calcularHcRequeridoPuesto, calcularHcRequeridoTotal } from "../../../modules/staffing";
...
function nuevoPuesto(ciudad:CiudadDemo):PuestoCotizado {
  ...
  return {
    id:newId(), tipoPuesto, nivelPerfil:"A", cantidadPosiciones:1, cobertura:"12x7", horas:12, dias:7, esquemaHoras:72, contingenciaPct:0,
    salarioMensual:base, uniformeCosto:0, equipoCosto:0, vehiculoOpcional:false, vehiculoCosto:0,
    examenes:[...EXAMENES_OBLIGATORIOS_IC], costoExamenesMensualizado:0,
  };
}
...
export function CalculadoraWizard() {
  ...
  const totalHcServicio=useMemo(()=>calcularHcRequeridoTotal(puestos),[puestos]);
  const simulacionEstructura=useMemo(()=>{
    if(!estructura) return null;
    const hcProyectado=estructura.hc+totalHcServicio;
  ...
  function selectClienteActual(id:string){
    ...
    setPuestos((prev)=>prev.map((p,i)=>i===0?{...p,tipoPuesto:c.cargo,nivelPerfil:"A",salarioMensual:c.salarioActual,examenes:[...EXAMENES_OBLIGATORIOS_IC]}:p));
  }
  ...
  function toggleExamen(id:string, examen:TipoExamen){
    if(EXAMENES_OBLIGATORIOS_IC.includes(examen)) return;
    const p=puestos.find((x)=>x.id===id); if(!p) return;
  ...
  return <div className="mx-auto max-w-6xl space-y-5">
    ...
    {step===1&&<div className="space-y-5">{puestos.map((p,idx)=>{const benchmark=benchmarkPara(datosGenerales.ciudad,p.tipoPuesto);const req=PERFIL_REQUISITOS[p.nivelPerfil??"A"];const cp=costosPorPuesto[p.id]??VACIO;const dim=calcularHcRequeridoPuesto(p);return <Card key={p.id} className="p-5"><div className="mb-4 flex items-center justify-between"><div><p className="font-semibold">Puesto {idx+1}</p><p className="text-xs text-ink-500">{req.etiqueta} · {req.experiencia}</p></div>{puestos.length>1&&<button className="text-xs text-danger-600" onClick={()=>removePuesto(p.id)}>Quitar</button>}</div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><FieldWrap label="Tipo de puesto"><SelectInput value={p.tipoPuesto} onChange={(e)=>{const tipo=e.target.value as PerfilPuesto;const base=benchmarkPara(datosGenerales.ciudad,tipo)?.recomendado??defaultSalario(tipo,datosGenerales.ciudad);updatePuesto(p.id,{tipoPuesto:tipo,salarioMensual:salarioPorNivel(base,p.nivelPerfil??"A")});}}>{PERFILES_COTIZADOR.map((x)=><option key={x.id} value={x.value}>{x.label}</option>)}</SelectInput></FieldWrap><FieldWrap label="Perfil / nivel"><SelectInput value={p.nivelPerfil??"A"} onChange={(e)=>changeNivel(p.id,e.target.value as NivelPerfil)}>{NIVELES.map((n)=><option key={n} value={n}>Perfil {n}</option>)}</SelectInput></FieldWrap><FieldWrap label="Posiciones"><TextInput type="number" min={1} value={p.cantidadPosiciones} onChange={(e)=>updatePuesto(p.id,{cantidadPosiciones:Number(e.target.value)})}/></FieldWrap><FieldWrap label="Turno / cobertura"><SelectInput value={p.cobertura} onChange={(e)=>{const t=TURNOS_DEMO.find((x)=>x.id===e.target.value);updatePuesto(p.id,{cobertura:e.target.value as PuestoCotizado["cobertura"],horas:t?.horas??p.horas,dias:t?.diasSemana??p.dias});}}>{TURNOS_DEMO.map((t)=><option key={t.id} value={t.id}>{t.label}</option>)}</SelectInput></FieldWrap><FieldWrap label="Esquema laboral (horas)" hint="Manual; admite decimales como 10.5"><TextInput type="number" min={0.5} step="0.5" value={p.esquemaHoras??72} onChange={(e)=>updatePuesto(p.id,{esquemaHoras:Math.max(0.5,Number(e.target.value)||72)})}/></FieldWrap><FieldWrap label="Contingencia %" hint="Temporalmente manual hasta cerrar fórmula oficial"><TextInput type="number" min={0} step="0.1" value={Math.round((p.contingenciaPct??0)*1000)/10} onChange={(e)=>updatePuesto(p.id,{contingenciaPct:Math.max(0,Number(e.target.value)/100)})}/></FieldWrap><FieldWrap label="Salario mensual"><TextInput type="number" value={p.salarioMensual} onChange={(e)=>updatePuesto(p.id,{salarioMensual:Number(e.target.value)})}/></FieldWrap></div><div className="mt-4 grid grid-cols-2 gap-3 rounded-2xl border border-brand-100 bg-brand-50/65 p-4 sm:grid-cols-4"><div><p className="text-[10px] font-semibold uppercase tracking-wide text-ink-400">Horas requeridas</p><p className="mt-1 text-lg font-semibold text-brand-800">{dim.horasSemana*p.cantidadPosiciones}</p></div><div><p className="text-[10px] font-semibold uppercase tracking-wide text-ink-400">Guardias base</p><p className="mt-1 text-lg font-semibold text-brand-800">{dim.hcBase}</p></div><div><p className="text-[10px] font-semibold uppercase tracking-wide text-ink-400">Contingencia</p><p className="mt-1 text-lg font-semibold text-warning-600">+{dim.hcContingencia}</p></div><div><p className="text-[10px] font-semibold uppercase tracking-wide text-ink-400">Total guardias</p><p className="mt-1 text-lg font-semibold text-success-600">{dim.hcTotal}</p></div><p className="col-span-2 text-[11px] text-ink-500 sm:col-span-4">Cálculo: horas del servicio ÷ esquema laboral ({dim.esquemaHoras} h), redondeado hacia arriba. La contingencia se agrega después sobre el HC base.</p></div>
      ...
      <div className="mt-4 rounded-xl border border-ink-200 p-4"><p className="text-sm font-semibold">Exámenes requeridos</p><p className="text-xs text-ink-500">Los marcados como Obligatorio IC forman parte del proceso base de contratación y no pueden retirarse.</p><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{EXAMENES_DEMO.map((e)=>{const obligatorio=e.obligatorioIc;const checked=obligatorio||(p.examenes??[]).includes(e.id);return <label key={e.id} className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-sm ${checked?"border-brand-300 bg-brand-50":"border-ink-200"} ${obligatorio?"cursor-not-allowed":"cursor-pointer"}`}><input type="checkbox" checked={checked} disabled={obligatorio} onChange={()=>toggleExamen(p.id,e.id)}/><span>{e.nombre}{obligatorio?<small className="block font-semibold text-brand-700">Obligatorio IC{e.costoReferencia!==null?` · ${money(e.costoReferencia)}/alta`:" · costo pendiente"}</small>:e.requiereValidacion?<small className="block text-warning-700">Opcional · costo pendiente de validar</small>:<small className="block text-ink-500">Opcional{e.costoReferencia!==null?` · ${money(e.costoReferencia)}/alta`:""}</small>}</span></label>})}</div>{(p.examenes??[]).includes("OTRO")&&<div className="mt-3 grid gap-3 rounded-xl border border-warning-200 bg-warning-50 p-3 sm:grid-cols-2"><FieldWrap label="Otro examen · Especifique"><TextInput value={p.otroExamenDescripcion??""} onChange={(e)=>updatePuesto(p.id,{otroExamenDescripcion:e.target.value})} placeholder="Ej. Toxicología ampliada"/></FieldWrap><FieldWrap label="Costo por alta (MXN)"><TextInput type="number" min={0} value={p.otroExamenCostoPorAlta??""} onChange={(e)=>updatePuesto(p.id,{otroExamenCostoPorAlta:Number(e.target.value)})}/></FieldWrap><p className="text-[11px] text-warning-700 sm:col-span-2">Este concepto se incorpora al costo y genera validación de Pricing por no pertenecer al catálogo base.</p></div>}{EXAMENES_DEMO.some((e)=>e.obligatorioIc&&e.costoReferencia===null)&&<div className="mt-3 rounded-lg border border-warning-200 bg-warning-50 px-3 py-2 text-xs text-warning-800"><b>Atención:</b> existe al menos un examen obligatorio IC sin costo cargado en el catálogo. La cotización podrá continuar, pero quedará pendiente de validación de Pricing hasta completar ese costo.</div>}</div>
      <div className="mt-4 space-y-3"><CatalogCostConfigurator tipo="UNIFORME" titulo="Uniformes" selecciones={cp.uniformes} maxCantidad={dim.hcTotal} onChange={(items)=>syncCostos(p.id,{...cp,uniformes:items})}/><CatalogCostConfigurator tipo="EQUIPO" titulo="Equipamiento" selecciones={cp.equipos} onChange={(items)=>syncCostos(p.id,{...cp,equipos:items})}/><CatalogCostConfigurator tipo="VEHICULO" titulo="Vehículos" selecciones={cp.vehiculos} onChange={(items)=>syncCostos(p.id,{...cp,vehiculos:items})}/></div>
