...
function buildCostBridge(puestos:PuestoCalculado[], costoMensualTotal:number) {
  ...
  // Desde V0.9 estos campos ya representan el total mensual capturado del grupo.
  const uniformes=sum((p)=>p.uniformeCosto);
  const equipo=sum((p)=>p.equipoCosto);
  const vehiculos=sum((p)=>(p.vehiculoOpcional?p.vehiculoCosto:0));
